...
UNIVERSAL_COLORS = {"black", "white"}

# Compact rules inspired by common complementary, analogous, and individual
# color pairings from the Bright Side color-combination guide.
COLOR_COMPATIBILITY = {
...
def normalize_color(color):
    ...
    normalized = unicodedata.normalize("NFKD", str(color).strip().lower())
    ascii_color = "".join(c for c in normalized if not unicodedata.combining(c))
    return COLOR_ALIASES.get(ascii_color, ascii_color)
...
def colors_compatible(color1, color2):
    ...
    if c1 == c2 or c1 in UNIVERSAL_COLORS or c2 in UNIVERSAL_COLORS:
        return True

    return c2 in COLOR_COMPATIBILITY.get(c1, set()) or c1 in COLOR_COMPATIBILITY.get(c2, set())


def has_compatibility_edges(graph):
    return any(values for values in graph.values())


def suggest_compatibility(garments):
    graph = {g.id: [] for g in garments}

    for index, garment in enumerate(garments):
        for other in garments[index + 1:]:
            if colors_compatible(getattr(garment, "color", None), getattr(other, "color", None)):
                graph[garment.id].append(other.id)
                graph[other.id].append(garment.id)
