...
- Add garments with type, color, warmth, and waterproof metadata.
- Choose manual garment combinations.
- Let the app suggest combinations from garment colors when no manual combinations are selected.
...
```text
.
+-- app.py               Local launcher for `python app.py`
...
- Builds a compatibility graph from manual combinations or color suggestions.
...
6. The frontend builds a compatibility graph from manual combinations or color suggestions.
...
The frontend lets users choose combinations manually. If no manual combinations are selected, the app builds suggested combinations from garment colors.

The color fallback uses a compact set of rules inspired by common complementary, analogous, and individual color pairings from Bright Side's color-combination guide:

```text
https://brightside.me/articles/the-ultimate-color-combinations-cheat-sheet-92405/
```

Manual combinations always take priority over suggested combinations.
...
- `compatibility`: saved manual garment combinations for the user.
...
4. For each garment, choose:
   ...
   - Warmth
   - Waterproof status
5. Select manual combinations, click `Suggest`, or leave combinations empty for color suggestions.
6. Click `Save garments` to persist the wardrobe and manual combinations.
7. Choose the target date and weather.
8. Click `Recommend outfit`.
9. Review the selected outfit and any blocked garments.
...
- User garment lists and manual combinations are stored per user.
- A login rotates the session token.
- The browser stores the current session in `localStorage`.
- Manual compatibility rules are saved per user.
- When no manual compatibility rules exist, the frontend and backend can infer suggested rules from garment colors.
...
- Adding automated tests for combination suggestions and manual pairing behavior.
...
Bienvenidos a mi portfolio de GitHub. En él iré subiendo proyectos personales que vaya desarrollando poco a poco. Los iré añadiendo a diferentes carpetas.
>>>>>>> 0fc8952493302ca288688f1cc9179aef23722b7c
