...
from engine import generate_outfits, has_compatibility_edges, select_best, suggest_compatibility
...
def load_garments(request: UserSession):
    try:
        return get_user_garments(request.username, request.token)
    except PermissionError as error:
        raise HTTPException(status_code=401, detail=str(error))
...
def save_garments(request: SaveGarmentsRequest):
    ...
    try:
        saved = save_user_garments(request.username, request.token, garments, request.compatibility)
    except PermissionError as error:
        raise HTTPException(status_code=401, detail=str(error))
...
def recommend_outfit(request: OutfitRequest):
    ...
    if not has_compatibility_edges(compatibility):
        compatibility = suggest_compatibility(request.garments)

    outfits = generate_outfits(request.garments, compatibility)
